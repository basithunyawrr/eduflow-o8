...
export default function ParentPortalPage() {
  ...
  const [messages, setMessages] = useState([{ from: 'ai', text: 'Assalam-o-Alaikum! Main EduFlow AI hoon. Sara ki parhai ya attendance ke baray me koi sawal poochein.' }])
  const [question, setQuestion] = useState('')
  const [typing, setTyping] = useState(false)

  const getAssistantReply = (prompt: string) => {
    const romanUrdu = /sara|math|parhai|attendance|kaisi|me|ki|hai|homework/i.test(prompt)
    if (/homework|test|upcoming|pending/i.test(prompt)) return romanUrdu ? 'Current Standing: 3 homework tasks pending hain aur Science ka test 2 September ko hai.\n\nAreas of Strength: Sara reading homework waqt par complete karti hai.\n\nActionable Advice: Roz 20 minutes homework routine banayein aur test se pehle short revision quiz karwain.' : 'Current Standing: Three homework tasks are pending, and the Science test is on September 2.\n\nAreas of Strength: Sara completes her reading work consistently.\n\nActionable Advice: Set a 20-minute homework routine and use a short revision quiz before the test.'
    if (/math|performance/i.test(prompt)) return romanUrdu ? 'Current Standing: Math average 88% hai; attendance 94% hai.\n\nAreas of Strength: Sara addition, subtraction aur class participation mein strong hai.\n\nActionable Advice: Ghar par 10 minutes fractions aur multi-step word problems practice karwayein.' : 'Current Standing: Sara’s Math average is 88% and attendance is 94%.\n\nAreas of Strength: She is strong in addition, subtraction, and class participation.\n\nActionable Advice: Practise fractions and multi-step word problems together for 10 minutes at home.'
    return romanUrdu ? 'Current Standing: Sara ki overall attendance 94% aur subject average 89% hai.\n\nAreas of Strength: Reading, Science aur class participation bohat achi hain.\n\nActionable Advice: Unki daily diary check karein, homework routine rakhein, aur choti progress ko celebrate karein.' : 'Current Standing: Sara has 94% attendance and an 89% subject average.\n\nAreas of Strength: Reading, Science, and class participation are going very well.\n\nActionable Advice: Check her daily diary, keep a gentle homework routine, and celebrate small wins at home.'
  }

  const sendQuestion = async (text = question) => {
    if (!text.trim() || typing) return
    const prompt = text.trim()
    setMessages((current) => [...current, { from: 'parent', text: prompt }])
    setQuestion('')
    setTyping(true)
    ...
    try {
      const response = await fetch('/api/ai/chat', {
        ...
        body: JSON.stringify({ message: prompt, studentName: 'Sara Ahmed', classContext: 'Class 5-A, Roll #14. Attendance 94%, subject average 89%, Math average 88%. Three homework tasks are pending. Science test is on September 2.' }),
      })
      const data = await response.json()
      if (!response.ok || typeof data.reply !== 'string') throw new Error('AI unavailable')
      setMessages((current) => [...current, { from: 'ai', text: data.reply }])
    } catch {
      setMessages((current) => [...current, { from: 'ai', text: getAssistantReply(prompt) }])
    } finally {
      setTyping(false)
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[#FFFCF3] text-slate-800">
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 lg:px-8 lg:py-10">
        ...
        {tab === 'ai' && <section className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-emerald-100 bg-white shadow-sm"><div className="bg-gradient-to-r from-[#FEF9C3] to-[#DCFCE7] p-5 sm:p-7"><div className="flex items-center gap-3"><span className="grid size-12 place-items-center rounded-2xl bg-white text-emerald-700 shadow-sm"><Sparkles size={22} /></span><div><h2 className="text-xl font-black">Ask EduFlow AI</h2><p className="text-sm text-slate-600">Your friendly guide to Sara&apos;s learning journey.</p></div><span className="ml-auto hidden rounded-full bg-white/70 px-3 py-1 text-xs font-bold text-emerald-700 sm:block">Live context</span></div><div className="mt-5 flex flex-wrap gap-2">{['Sara ki Attendance summary', 'Aj ka homework kya hai?', 'Next exam syllabus'].map(prompt => <button key={prompt} onClick={() => sendQuestion(prompt)} disabled={typing} className="rounded-full bg-white/80 px-3 py-2 text-xs font-bold text-slate-700 transition hover:bg-white disabled:opacity-50">{prompt}</button>)}</div></div><div className="min-h-72 space-y-4 bg-white p-5 sm:p-7">{messages.map((message, i) => <div key={i} className={`flex items-end gap-2 ${message.from === 'parent' ? 'justify-end' : 'justify-start'}`}>{message.from === 'ai' && <span className="grid size-8 shrink-0 place-items-center rounded-full bg-[#FEF9C3] text-amber-800"><Sparkles size={15} /></span>}<div className={`max-w-[85%] whitespace-pre-line rounded-2xl px-4 py-3 text-sm leading-6 ${message.from === 'parent' ? 'bg-slate-900 text-white' : 'bg-[#DCFCE7] text-slate-700'}`}>{message.text}</div>{message.from === 'parent' && <span className="grid size-8 shrink-0 place-items-center rounded-full bg-slate-100 text-slate-600"><UserRound size={15} /></span>}</div>)}{typing && <div className="flex items-center gap-2 text-sm text-slate-400"><span className="grid size-8 place-items-center rounded-full bg-[#FEF9C3] text-amber-800"><Sparkles size={15} /></span><span className="rounded-2xl bg-slate-50 px-4 py-3">EduFlow AI is typing...</span></div>}</div><div className="border-t border-slate-100 bg-white p-5 sm:px-7"><div className="mb-4 flex items-center gap-2 rounded-2xl bg-slate-50 p-2"><button aria-label="Voice query" className="grid size-10 place-items-center rounded-xl text-emerald-700 hover:bg-[#DCFCE7]"><Mic size={17} /></button><input value={question} disabled={typing} onChange={(e) => setQuestion(e.target.value)} onKeyDown={(e) => { if (e.nativeEvent.isComposing || e.keyCode === 229) return; if (e.key === 'Enter') sendQuestion() }} placeholder="Ask in English or Roman Urdu..." className="min-w-0 flex-1 bg-transparent px-2 text-sm outline-none" /><button onClick={() => sendQuestion()} disabled={typing} aria-label="Send question" className="grid size-10 place-items-center rounded-xl bg-slate-900 text-white transition hover:bg-emerald-700 disabled:opacity-50"><Send size={16} /></button></div><button className="inline-flex items-center gap-2 rounded-full bg-[#FEF9C3] px-4 py-2.5 text-xs font-black text-slate-800 transition hover:bg-[#DCFCE7]"><Download size={15} /> Download Performance Card (PDF)</button></div></section>}
      </div>
